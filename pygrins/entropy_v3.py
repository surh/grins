...
def calculate_compression_ratio(seq):
    ...
    seq_c = zlib.compress(seq.encode('utf-8'), 9)

    # The minus 8 is the decompression key string length.
    return float(len(seq_c) - 8) / float(len(seq))
...
#         w_counts = np.fromiter(Kmers.values(), dtype=float)
...
def sw_kmer_shannon(seq, start=0, step=5, window=20, k=3):
    ...
    for start in range(start, len(seq) - k + 1):
        kmer = seq[start:start+k]
    ...
    for start in range(0, len(seq) - k - start + 1):
        kmer = seq_revcomp[start:start+k]
