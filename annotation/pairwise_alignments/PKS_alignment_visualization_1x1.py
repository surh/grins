...
for i in range(0,args.splitsize1,30):
	...
	for j in range(0,splitsize2,30):
...
with open(args.output_folder+record_name1+"_vs_"+record_name2+"_window150_step30_1x1.txt",'w') as output_file:
    for i in range(0,len(homology_matrix)):
        for j in range(0,len(homology_matrix[i])):
            output_file.write(str(homology_matrix[i][j])+",")
        output_file.write("\n")

print "alignment 1x1 finished"
print time.ctime()      